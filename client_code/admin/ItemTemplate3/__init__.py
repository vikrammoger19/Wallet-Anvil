...
class ItemTemplate3(ItemTemplate3Template):
  def __init__(self,  **properties):
    ...
    existing_img = self.item['users_profile_pic']
  ...
  def button_1_click(self, **event_args):
    ...
    open_form('admin.admin_view', phone_number=phone_number,user=selected_user)

  def check_status(self):
      now = datetime.now(anvil.tz.tzlocal())
  # Parse the 'last_login' date string into a datetime object
      last_login = datetime.strptime(str(self.item['users_last_login']), '%Y-%m-%d %H:%M:%S.%f%z') 
